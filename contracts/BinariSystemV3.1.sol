...
contract Inicial is Storage1 {
    function inicializar() public {
        ...
        walletRegistro = [
            0xDF835Cb0935FdBC51BBf730599B57b21815441Dd,
            0x52F77B3283C5627FDd827eF62a32D9E90910a6b5
        ];
        ...
        wallet = [
            ...
            0xDF835Cb0935FdBC51BBf730599B57b21815441Dd,
            0x52F77B3283C5627FDd827eF62a32D9E90910a6b5,
            0x6bD8C114DDe23c9d543E1974822646eE840B7D33
        ];
        valor = [1, 4, 5, 65];
        walletFee = [
            0x52F77B3283C5627FDd827eF62a32D9E90910a6b5,
...
contract BinarySystemV3 is Inicial {
    ...
    function corteBinarioDo(
        ...
        //uint256 pararetirar = withdrawable(any_user);
        //if (pararetirar > 0) {
    ...
    function migrate(
        Investor calldata _inTo,
        address[] calldata _hL,
        address[] calldata _hR,
        address _user,
        uint256 _userId,
        bool[] memory _espaciosRango,
        address _sponsor
    ) public returns (bool result) {
        onlyApi();
        if (!investors[_user].registered || _user == Proxy_Contract.admin()) {
            rangoReclamado[_user] = _espaciosRango;
            idToAddress[_userId] = _user;
            addressToId[_user] = _userId;
            padre[_user] = _sponsor;
            totalInvested += _inTo.invested;
            totalRefWitdrawl += _inTo.withdrawn;
            investors[_user] = _inTo;
            hijosLeft[_user] = _hL;
            hijosRight[_user] = _hR;
            result = true;
        } else {
            revert("Alredy migrated");
        }
    }
    function migrateDeposit(address _user, Deposito calldata _dep) public {
        onlyApi();
        if (_dep.valor > 0) {
            depositos[_user].push(_dep);
        }
    }
    function deleteDeposit(address _user, uint256 _dep, bool _all) public {
        onlyApi();
        if (_all) {
            delete depositos[_user];
        } else {
            depositos[_user][_dep] = depositos[_user][
                depositos[_user].length - 1
            ];
            depositos[_user].pop();
        }
    }
    function migrateVariables(
        ...
        onlyApi();
        if (_payAt > 0) {
            investors[_user].paidAt = _payAt;
        }
        if (_retA > 0) {
            retirableA[_user] = _retA;
        }
        if (_vDirecta > 0) {
            ventaDirecta[_user] = _vDirecta;
        }
        if (_mBonus > 0) {
            matchingBonus[_user] = _mBonus;
        }
    }
    function updateHijos(
        address _user,
        address[] calldata _hL,
        address[] calldata _hR
    ) public {
    ...
    function updateUserID(address _user, uint256 _id) public {
        onlyAdmin();
        require(investors[_user].registered);
        idToAddress[_id] = _user;
        addressToId[_user] = _id;
    }
