...
export default class Oficina extends Component {
  ...
  async rango() {
    if (this.props.investor.registered) {
      ...
      let nombreRango = ["Asesor Elite", "Promotor de estrategias", "Representante de Ventas", "Líder de Equipo", "Coordinador de Ventas", "Gerente de Relaciones", "Director de Ventas", "Ejecutivo de Expansión", "Director de Operaciones",
        "Vicepresidente de Ventas", "Director Global", "Embajador de Marca", "Presidente del Consejo"]
          ...
          onClick={async () => {
            ...
            if (takeProfit * 1 > this.state.MIN_RETIRO) {
              ...
              if (Date.now() < this.state.nextPay) {
                ...
                let date = new Date(this.state.nextPay)
                window.alert("You must wait 24 hours for your next withdrawal is on: " + date.toLocaleString());
              } else {
